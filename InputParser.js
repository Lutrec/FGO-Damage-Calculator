...
const BASE_MODS = "hp|lv|str|oc|m|cp|a|d|am|bm|qm|em|n|p|cd|sdm|sam|se|fd|ce|f|fp|npp|ap|bp|qp|ep|acd|bcd|qcd|np|npo|ng|ang|bng|qng|esm|esr|sg|asg|bsg|qsg|ta|cao|aao|cmo|fr|fs|ecm|rng|reducedhp|ff|dr|ngr|sgr";
const ATTRIBUTES = new Set(["man", "sky", "earth", "star", "beast"]);
const CLASSES = new Set([
  "saber", "archer", "lancer", "rider", "caster", "assassin", "berserker",
  "shielder", "ruler", "avenger", "mooncancer", "mooncancerciel", "alterego",
  "alteregokiara", "foreigner", "pretender", "beast", "beast1", "beast1lost",
  "beast2", "beast3r", "beast3l", "beast4", "beast6", "beastdraco", "beasteresh", "beastolga",
]);
const FLAGS = new Set([
  "crit", "np", "b", "q", "e", "second", "third", "card1", "card2", "card3", "card4", "global",
  "mighty", "ok", "af", "qf", "bf", "nobf", "bc", "nocap",
]);
...
export const InputParser = {
  ...
  getPattern() {
    ...
    return new RegExp(`^(?<type>${allBaseKeys}${aliasString}${macroString})(?<value>-?\\d*\\.?\\d+)?$`, "i");
  },
    ...
    while (queue.length > 0) {
      ...
      if (match && match.groups) {
        ...
        if (ATTRIBUTES.has(type)) buffs.enemyAttribute = type;
          ...
          if (["nobf", "af", "qf", "bf", "bc", "nocap", "global"].includes(type)) {
            buffs.flags[type] = true;
            currentPos = 0; 
          } else if (type.startsWith("card")) {
            currentPos = parseInt(type.substring(4)) || 0;
          } else {
            currentPos > 0 ? (buffs.cardFlags[currentPos][type] = true) : (buffs.flags[type] = true);
          }
        } else {
          ...
          if (!isNaN(val)) {
            if (type === "hp") buffs.enemyHp = val;
            ...
            else if (type === "np") buffs.npLevel = Math.max(1, Math.min(5, Math.floor(val)));
            else if (type === "oc") {
              ...
              const targetMap = currentPos > 0 ? buffs.cardMods[currentPos] : buffs.mods;
              if (type === "m") {
                ["am", "bm", "qm"].forEach((t) => (targetMap[t] = (targetMap[t] || 0) + val));
              } else if (type === "cp") {
                ["ap", "bp", "qp"].forEach((t) => (targetMap[t] = (targetMap[t] || 0) + val));
